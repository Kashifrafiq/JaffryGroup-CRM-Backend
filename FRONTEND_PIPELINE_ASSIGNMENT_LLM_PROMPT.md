...
Effect: assigns that associate to ALL pipeline steps of the first application created with the customer.
```
...
- Detail/workflow: same customer endpoints; 403 if no step assignment

**Pipeline step actions for associate:**

- Show all steps
- Enable “Mark complete” only if logged-in associate’s profile `id` is in `step.assignedTo`
- Hide all assign/unassign controls

### Screen 4: Document workflow (same application context)

Associates with access to **any** step on an application can use document endpoints for that application:
