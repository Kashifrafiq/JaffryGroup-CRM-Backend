...
export class NormalizeDeprecatedSchema1740000000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (uploadedByColumn) {
      ...
      const hasFk = documentsTable?.foreignKeys.some((fk) =>
        fk.columnNames.includes('uploadedByUserId'),
      );
      if (!hasFk) {
        await queryRunner.query(`
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE customer_applications ADD COLUMN IF NOT EXISTS pipeline jsonb`,
    );
    await queryRunner.query(
      `ALTER TABLE customers ADD COLUMN IF NOT EXISTS "applicationType" varchar(128)`,
    );
